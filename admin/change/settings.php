...
if(isset($_POST['resetdefault']) && $_POST['resetdefault'] != 'yes') { 
    ...
    */
    ...
    if(isset($_POST['HWI_BRANDING']) && $config["HWI_BRANDING"] != $_POST['HWI_BRANDING']) {
        $_branding = mysqli_real_escape_string($conn, $_POST['HWI_BRANDING']);
        $query .= "UPDATE `".$mysql_table."config` SET `VALUE` = '".$_branding."' WHERE `VARIABLE` = 'HWI_BRANDING';";
    } 
    ...
    if(isset($_POST['HESTIA_HOST_ADDRESS']) && $config["HESTIA_HOST_ADDRESS"] != $_POST['HESTIA_HOST_ADDRESS']) {
        $_host_address = mysqli_real_escape_string($conn, $_POST['HESTIA_HOST_ADDRESS']);
        $query .= "UPDATE `".$mysql_table."config` SET `VALUE` = '".$_host_address."' WHERE `VARIABLE` = 'HESTIA_HOST_ADDRESS';";
    } 
    if(isset($_POST['HESTIA_SSL_ENABLED']) && $config["HESTIA_SSL_ENABLED"] != $_POST['HESTIA_SSL_ENABLED']) { 
        $_ssl_enabled = mysqli_real_escape_string($conn, $_POST['HESTIA_SSL_ENABLED']);
        $query .= "UPDATE `".$mysql_table."config` SET `VALUE` = '".$_ssl_enabled."' WHERE `VARIABLE` = 'HESTIA_SSL_ENABLED';";
    } 
    if(isset($_POST['HESTIA_PORT']) && $config["HESTIA_PORT"] != $_POST['HESTIA_PORT']) { 
        $_port = mysqli_real_escape_string($conn, $_POST['HESTIA_PORT']);
        $query .= "UPDATE `".$mysql_table."config` SET `VALUE` = '".$_port."' WHERE `VARIABLE` = 'HESTIA_PORT';";
    } 
    if(isset($_POST['HESTIA_METHOD']) && $config["HESTIA_METHOD"] != $_POST['HESTIA_METHOD']) { 
        $_method = mysqli_real_escape_string($conn, $_POST['HESTIA_METHOD']);
        $query .= "UPDATE `".$mysql_table."config` SET `VALUE` = '".$_method."' WHERE `VARIABLE` = 'HESTIA_METHOD';";
    } 
    if(isset($_POST['HESTIA_API_KEY']) && $vst_apikey != $_POST['HESTIA_API_KEY']) { 
        $enckey = hwicryptx($_POST['HESTIA_API_KEY']);
        $_apikey = mysqli_real_escape_string($conn, $enckey);
        $query .= "UPDATE `".$mysql_table."config` SET `VALUE` = '".$_apikey."' WHERE `VARIABLE` = 'HESTIA_API_KEY';";
    } 
    if(isset($_POST['HESTIA_ADMIN_UNAME']) && $vst_username != $_POST['HESTIA_ADMIN_UNAME']) { 
        $_admin_uname = mysqli_real_escape_string($conn, $_POST['HESTIA_ADMIN_UNAME']);
        $query .= "UPDATE `".$mysql_table."config` SET `VALUE` = '".$_admin_uname."' WHERE `VARIABLE` = 'HESTIA_ADMIN_UNAME';";
    } 
    if(isset($_POST['HESTIA_ADMIN_PW']) && $vst_password != $_POST['HESTIA_ADMIN_PW']) { 
        $encpassword = hwicryptx($_POST['HESTIA_ADMIN_PW']);
        $_admin_pw = mysqli_real_escape_string($conn, $encpassword);
        $query .= "UPDATE `".$mysql_table."config` SET `VALUE` = '".$_admin_pw."' WHERE `VARIABLE` = 'HESTIA_ADMIN_PW';";
    }
...
// Reset To Default
    ...
    $query .= "UPDATE `".$mysql_table."config` SET `VALUE` = 'true' WHERE `VARIABLE` IN ('HWI_BRANDING', 'DEFAULT_TO_ADMIN', 'ADMIN_ADS', 'WEB_ENABLED', 'DNS_ENABLED', 'MAIL_ENABLED', 'DB_ENABLED', 'ADMIN_ENABLED', 'PROFILE_ENABLED', 'CRON_ENABLED', 'BACKUPS_ENABLED', 'NOTIFICATIONS_ENABLED', 'SMTP_ENABLED', 'SMTP_AUTH');";
    $query .= "UPDATE `".$mysql_table."config` SET `VALUE` = 'false' WHERE `VARIABLE` IN ('CUSTOM_FOOTER', 'REGISTRATIONS_ENABLED', 'OLD_CP_LINK', 'PHPMAIL_ENABLED');";
